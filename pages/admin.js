...
export default function Admin() {
  ...
  const [vista,    setVista]    = useState('proximas') // proximas | todas | hoy
      ...
      <div className="ap">
        ...
        <div className="astats">
          {[
            ...
            ['Hoy',   deHoy.length,    '#dcfce7', '#16a34a'],
            ['Próximas', proximas.length, '#f0fdf4', '#15803d'],
                    ...
                    {mostrar.map(b => {
                      ...
                      return (
                        <tr key={b.id} className={esHoy ? 'atrHoy' : esFut ? 'atrFut' : 'atr'}>
                          <td className="atd">
                            <span className={`fecha-pill ${esHoy ? 'hoy' : esFut ? 'fut' : 'pas'}`}>
                              {b.date}
                            </span>
                          </td>
...
const css = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  .tab{padding:8px 16px;border-radius:10px;border:1.5px solid #e5e7eb;background:#fff;font-size:13px;font-weight:500;color:#6b7280;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all .15s;}
